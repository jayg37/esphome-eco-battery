...
async def to_code(config):
    ...
    if CONF_SOC_SENSOR in config:
        sens = await cg.get_variable(config[CONF_SOC_SENSOR])
    ...
    if CONF_MOS_TEMP_SENSOR in config:
        sens = await cg.get_variable(config[CONF_MOS_TEMP_SENSOR])
        cg.add(var.set_mos_temp_sensor(sens))

    if CONF_CELL_TEMP_SENSOR in config:
        sens = await cg.get_variable(config[CONF_CELL_TEMP_SENSOR])
    ...
    if CONF_MAX_CELL_VOLTAGE_SENSOR in config:
        sens = await cg.get_variable(config[CONF_MAX_CELL_VOLTAGE_SENSOR])
        cg.add(var.set_max_cell_voltage_sensor(sens))

    if CONF_MIN_CELL_VOLTAGE_SENSOR in config:
        sens = await cg.get_variable(config[CONF_MIN_CELL_VOLTAGE_SENSOR])
        cg.add(var.set_min_cell_voltage_sensor(sens))

    if CONF_CELL_DELTA_SENSOR in config:
        sens = await cg.get_variable(config[CONF_CELL_DELTA_SENSOR])
        cg.add(var.set_cell_delta_sensor(sens))

    if CONF_CELL_COUNT_SENSOR in config:
        sens = await cg.get_variable(config[CONF_CELL_COUNT_SENSOR])
        cg.add(var.set_cell_count_sensor(sens))

    if CONF_TEMP_PROBE1_SENSOR in config:
        sens = await cg.get_variable(config[CONF_TEMP_PROBE1_SENSOR])
        cg.add(var.set_temp_probe1_sensor(sens))

    if CONF_TEMP_PROBE2_SENSOR in config:
        sens = await cg.get_variable(config[CONF_TEMP_PROBE2_SENSOR])
        cg.add(var.set_temp_probe2_sensor(sens))

    if CONF_TEMP_PROBE3_SENSOR in config:
        sens = await cg.get_variable(config[CONF_TEMP_PROBE3_SENSOR])
